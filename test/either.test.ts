...
test('Either.right does not throw', () => {
  ...
  expect(() =>
    either
      .map(x => x * x)
      .map(x => x - 10)
      .getOrElseThrow((val: number) => new Error())
  ).not.toThrow();
});
...
test('Either.left is chainable', () => {
  ...
  const value = either
    .map((x: string) => x.length * x.length)
    .filter((x: number) => x >= 90)
...
test('Either.nullable is mappable', () => {
  ...
  const retrieveVal = either
    .map((x: string) => x.length)
    .map((x: number) => `The length of your value is ${x}`)
    .map((x: string) => x.toUpperCase());

  expect(retrieveVal.getOrElse('An error occurred retrieving your value')).toBe('THE LENGTH OF YOUR VALUE IS 7');
});
...
test('Either.nullable is filterable', () => {
  ...
  expect(retrieveVal.getOrElse('An error occurred retrieving your value')).toBe(
    'An error occurred retrieving your value'
  );
});
