...
describe('safeUnpackG', () => {
    ...
    co.wrap(function* () {
      const profileRetrievalWithError: <T>(val: string) => any = curry(function* (
        ...
        return yield Promise.resolve(
          Either.nullable({ error: new Error(`Missing profile with id: ${val}`), data: null })
        );
      });
      ...
      const safelyGetProfile = compose(
        safeUnpackG(
          ...
          'Unfortunately the following error occurred: Missing profile with id: a-b-c-d-e'
        ),
        profileRetrievalWithError
      );
  ...
  test(
    ...
    co.wrap(function* () {
      ...
      expect(profile.data).toStrictEqual({
        id: 'a-b-c-d-e',
