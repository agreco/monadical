...
test('Maybe.nothing is filterable', () => {
  ...
  const xVal: Maybe<number, void> =
    xa.map((x: number) => x*x).map((x: number) => (x - 10)).filter((x: number) => x >= 90);
  
  expect(xVal.isNothing()).toBe(true);

  const ya: Maybe<number, void> = Maybe.nothing<number, void>();
  const yVal: number | Nothing<number, void> =
    ya.map((x: number) => x*x).map((x: number) => (x - 10)).filter((x: number) => x >= 90).chain((x: number) => x * 2);
...
test('Maybe.nothing is chainable', () => {
  ...
  const xVal: number | Nothing<number, void> =
    xa.map((x: number) => x*x).map((x: number) => (x - 10)).chain((x: number) => x * 2);
